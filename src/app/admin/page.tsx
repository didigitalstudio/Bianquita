...
import { PRODUCTS as INITIAL_PRODUCTS, INITIAL_ORDERS, CATEGORIES, AUDIENCES, fmt } from "@/lib/data";
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "100vh", background: "#FAF6EE" }}>
      ...
      <div style={{ padding: "32px 40px", overflow: "auto" }}>
        ...
        {view === "products" && <AdminProducts products={products} setProducts={setProducts} setView={setView} setEditId={setEditProductId} showToast={showToast} />}
        {view === "edit" && <AdminEditProduct products={products} setProducts={setProducts} setView={setView} productId={editProductId} showToast={showToast} />}
...
function AdminDashboard({ products, orders, setView }: { products: Product[]; orders: Order[]; setView: (v: AdminView) => void }) {
  ...
  const STATUS_COLORS: Record<string, string> = { "pendiente-pago": "badge-warn", preparando: "badge-soft", enviado: "badge-salvia", entregado: "badge-ok" };
  const STATUS_LABELS: Record<string, string> = { "pendiente-pago": "Pendiente pago", preparando: "Preparando", enviado: "En camino", entregado: "Entregado" };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 20, marginBottom: 32 }}>
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
            ...
            <div style={{ textAlign: "right" }}><div style={{ fontFamily: "var(--font-display)", fontSize: 26 }}>{fmt(totalSales)}</div><div style={{ fontSize: 12, color: "var(--salvia-deep)", fontWeight: 600 }}>↑ 12.4%</div></div>
          </div>
...
function AdminProducts({ products, setProducts, setView, setEditId, showToast }: { products: Product[]; setProducts: (p: Product[]) => void; setView: (v: AdminView) => void; setEditId: (id: string) => void; showToast: (m: string) => void }) {
  ...
  const remove = (id: string) => { if (!confirm("¿Eliminar este producto?")) return; setProducts(products.filter((p) => p.id !== id)); showToast("Producto eliminado"); };
  const edit = (id: string) => { setEditId(id); setView("edit"); };
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <button className="btn btn-primary" onClick={() => { setEditId("new"); setView("edit"); }}><Icon name="plus" size={14} /> Nuevo producto</button>
      </div>
          ...
          return (
            <div key={p.id} style={{ display: "grid", gridTemplateColumns: "80px 2fr 1fr 1fr 1fr 1fr 100px", padding: "14px 20px", borderTop: "1px solid var(--line-soft)", alignItems: "center", fontSize: 14 }}>
              ...
              <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                <button className="btn-icon" style={{ width: 32, height: 32 }} onClick={() => edit(p.id)}><Icon name="edit" size={14} /></button>
                <button className="btn-icon" style={{ width: 32, height: 32, color: "#a55" }} onClick={() => remove(p.id)}><Icon name="trash" size={14} /></button>
              </div>
            </div>
          );
        })}
...
function AdminEditProduct({ products, setProducts, setView, productId, showToast }: { products: Product[]; setProducts: (p: Product[]) => void; setView: (v: AdminView) => void; productId: string | null; showToast: (m: string) => void }) {
  const isNew = !productId || productId === "new";
  const existing = products.find((p) => p.id === productId);
  const blank: Product = { id: "p" + Date.now(), name: "", category: "bodies", audience: "bebe", price: 0, compareAt: null, stock: {}, colors: [], description: "", materials: "", care: "", tags: [], img: "" };
...
function AdminStock({ products, setProducts }: { products: Product[]; setProducts: (p: Product[]) => void; showToast?: (m: string) => void }) {
  const [search, setSearch] = useState("");
  ...
  const updStock = (pid: string, size: string, val: string) => setProducts(products.map((p) => p.id === pid ? { ...p, stock: { ...p.stock, [size]: Math.max(0, +val || 0) } } : p));
  const adjustStock = (pid: string, size: string, delta: number) => { const p = products.find((x) => x.id === pid)!; updStock(pid, size, String((p.stock[size] || 0) + delta)); };
...
function AdminOrders({ orders, setOrders, showToast }: { orders: Order[]; setOrders: (o: Order[]) => void; showToast: (m: string) => void }) {
  const [filter, setFilter] = useState("all");
        ...
        <div onClick={() => setSelected(null)} style={{ position: "fixed", inset: 0, background: "rgba(43,35,29,0.45)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: 24, animation: "fadeIn .2s" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--paper)", borderRadius: 22, width: 640, maxHeight: "90vh", overflowY: "auto", animation: "slideUp .3s" }}>
            <div style={{ padding: "24px 28px", borderBottom: "1px solid var(--line-soft)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><div style={{ fontSize: 12, color: "var(--ink-mute)" }}>Pedido</div><div className="h3">{selected.id}</div></div>
              <button className="btn-icon" onClick={() => setSelected(null)}><Icon name="x" /></button>
            </div>
            <div style={{ padding: 28 }}>
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {Object.entries(STATUS).map(([id, s]) => (
                    <button key={id} onClick={() => updateStatus(selected.id, id as Order["status"])} style={{ padding: "8px 14px", borderRadius: 999, border: selected.status === id ? "2px solid var(--brand)" : "1px solid var(--line)", background: selected.status === id ? "var(--brand-soft)" : "#fff", fontSize: 13, fontWeight: 600 }}>

// === CART DRAWER ===
const CartDrawer = ({ open, onClose, cart, updateQty, removeFromCart, navigate }) => {
  if (!open) return null;
      ...
      <div onClick={e=>e.stopPropagation()} style={{ position: "absolute", right: 0, top: 0, height: "100%", width: 460, maxWidth: "94vw", background: "var(--paper)", display: "flex", flexDirection: "column", animation: "slideLeft .3s ease-out" }}>
        ...
        {cart.length === 0 ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center" }}>
            <EmptyState icon="bag" title="Tu carrito está vacío" body="Agregá prendas para empezar tu compra." cta={<button className="btn btn-primary" onClick={()=>{onClose();navigate("shop");}}>Ver productos</button>}/>
          </div>
        ) : (
          <>
            {!freeShip && (
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 28px" }}>
              {cart.map((item, idx) => (
                <div key={idx} style={{ display: "grid", gridTemplateColumns: "80px 1fr auto", gap: 14, padding: "16px 0", borderBottom: "1px solid var(--line-soft)" }}>
                  <div style={{ width: 80, height: 96, borderRadius: 10, overflow: "hidden", background: "var(--cream)" }}>
                    <ProductImage src={item.img} alt={item.name} label={item.category}/>
                  </div>
                  <div>
            ...
            <div style={{ padding: "20px 28px 28px", borderTop: "1px solid var(--line)" }}>
              ...
              <button className="btn btn-primary btn-lg" style={{ width: "100%", marginBottom: 8 }} onClick={()=>{onClose();navigate("checkout");}}>Finalizar compra →</button>
              <button className="btn btn-ghost" style={{ width: "100%" }} onClick={()=>{onClose();navigate("shop");}}>Seguir comprando</button>
            </div>
          </>
        )}
      </div>

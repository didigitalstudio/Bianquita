// === CHECKOUT ===
const Checkout = ({ navigate, cart, clearCart, showToast }) => {
  ...
  const shippingCost = subtotal >= 35000 ? 0 : (shipping==="cadeteria"?2500:shipping==="andreani-sucursal"?3800:5400);
  const total = subtotal + shippingCost;

  if (cart.length === 0 && !done) {
    return (
      <div className="container" style={{ padding: "120px 24px" }}>
        <EmptyState icon="bag" title="Tu carrito está vacío" body="Agregá productos antes de pagar." cta={<button className="btn btn-primary" onClick={()=>navigate("shop")}>Ir a la tienda</button>}/>
      </div>
    );
  }
    ...
    return (
      <div className="container" style={{ padding: "80px 24px", maxWidth: 640 }}>
        <div style={{ textAlign: "center", padding: "48px 32px", background: "var(--cream)", borderRadius: 22 }}>
          ...
          <p className="muted" style={{ marginBottom: 8 }}>Pedido <strong style={{ color: "var(--brand)" }}>ULB-{Math.floor(Math.random()*9000+1000)}</strong></p>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button className="btn btn-primary" onClick={()=>navigate("account")}>Ver mis pedidos</button>
            ...
            <div className="card" style={{ padding: 28 }}>
              ...
              {payment === "card" && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  ...
                  <div className="field" style={{ gridColumn: "span 2" }}><label>Cuotas</label><select className="select"><option>1 cuota de {fmt(total)}</option><option>3 cuotas sin interés de {fmt(Math.round(total/3))}</option><option>6 cuotas de {fmt(Math.round(total/6))}</option><option>12 cuotas de {fmt(Math.round(total/12))}</option></select></div>
                </div>
              )}
              {payment === "transfer" && (
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 24 }}>
                ...
                <button className="btn btn-primary" onClick={()=>{ clearCart(); setDone(true); }}>Pagar {fmt(total)}</button>
              </div>
            </div>
          )}
        </div>
            ...
            <div style={{ maxHeight: 280, overflowY: "auto", marginBottom: 16 }}>
              {cart.map((it, i) => (
                <div key={i} style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 56, height: 64, borderRadius: 8, overflow: "hidden", background: "#fff", flexShrink: 0 }}>
